...
class GraphRetrieverSettings(BaseModel):
    model_config = ConfigDict(extra="forbid")

    graph_name: str = Field(..., description="Logical Neo4j graph name to query.")
    limit: int = Field(default=8, ge=1, le=100, description="Maximum number of references returned after graph ranking.")
    seed_limit: int = Field(default=5, ge=1, le=50, description="Number of fulltext seed references used to start graph expansion.")
    neighbor_depth: int = Field(default=1, ge=0, le=10, description="How many PREV/NEXT hops are considered around each seed.")
    include_section_references: bool = Field(
        default=True,
        description="Whether references from the same section as a seed should be added.",
    )
    section_max_children: int = Field(
        default=24,
        ge=1,
        le=500,
        description="Soft cap used when section-based expansion fan-out is large.",
    )
    ...
    keyword_overlap_min: int = Field(
        ...
        ge=1,
        le=20,
        description="Minimum number of shared canonical keywords required for keyword-based expansion when enabled.",
    )
