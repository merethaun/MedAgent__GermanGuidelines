...
class GuidelineDownloadInformation(BaseModel):
    ...
    download_date: Optional[datetime] = Field(
        default=None, description="Timestamp when the guideline was downloaded",
        examples=["2025-04-14T16:05:25.380705"],
    )
    file_path: Optional[str] = Field(
        default=None, description="Local path to the downloaded guideline PDF",
        examples=["output/guideline/pdf/007-106l_S3_Totaler_alloplastischer_Kiefergelenkersatz.pdf"],
    )
    page_count: Optional[int] = Field(
        default=None, description="Total page count of the guideline PDF", examples=[152],
    )
...
class GuidelineValidityInformation(BaseModel):
    ...
    extended_validity: bool = Field(
        ..., description="True if validity was officially extended beyond original expiration", examples=[False],
    )
    validity_range: int = Field(
        default=5, description="Validity range in years; default is 5 years, 1 year for living guidelines",
        examples=[5],
    )
...
class GuidelineEntry(BaseModel):
    ...
    title: str = Field(
        ..., description="Official title of the guideline", examples=["Totaler alloplastischer Kiefergelenkersatz"],
    )
    ...
    keywords: List[str] = Field(
        default_factory=list, description="Keywords related to the guideline (manually assigned or extracted)",
        examples=[["Kiefergelenkprothese", "Komplikationen", "jaw replacement"]],
    )
    goal: Optional[str] = Field(
        None, description="Goal or general description of the guideline",
        examples=[
    ...
    target_patients: Optional[str] = Field(
        None, description="Target patient group for the guideline",
        examples=["Zielgruppe sind alle Menschen mit Weisheitszähnen", "Kinder und Erwachsene"],
    )
    care_area: Optional[str] = Field(
        None, description="Medical care context or area the guideline applies to",
        examples=[
        ...
        json_schema_extra={
            "example": {
                ...
                "validity_information": {
                    "guideline_creation_date": "2025-04-01T00:00:00", "valid": False, "extended_validity": False,
                    "validity_range": 5,
                },
            },
        }, )
